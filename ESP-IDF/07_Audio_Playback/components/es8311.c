...
static esp_err_t codec_i2c_write(unsigned int port, uint16_t addr,
                                 const uint8_t *data, size_t len)
{
    ...
    if (err == ESP_OK) err = i2c_master_write(cmd, (uint8_t *)data, len, true);
    if (err == ESP_OK) err = i2c_master_stop(cmd);
    if (err == ESP_OK) err = i2c_master_cmd_begin((i2c_port_t)port, cmd, pdMS_TO_TICKS(1000));
    i2c_cmd_link_delete(cmd);
    return err;
}

static esp_err_t codec_i2c_write_read(unsigned int port, uint16_t addr,
                                      const uint8_t *wdata, size_t wlen,
                                      uint8_t *rdata, size_t rlen)
{
    return i2c_master_write_read_device((i2c_port_t)port, addr,
...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return codec_i2c_write(es->port, es->dev_addr, write_buf, sizeof(write_buf));
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    return codec_i2c_write_read(es->port, es->dev_addr, &reg_addr, 1, reg_value, 1);
}
